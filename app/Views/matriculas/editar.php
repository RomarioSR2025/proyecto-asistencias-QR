...
<div class="container mt-4">
  ...
  <form action="<?= base_url('matriculas/actualizar/'.$matricula['idmatricula']); ?>" method="post" class="mt-3">
      ...
      <select name="idalumno" id="idalumno" class="form-select" required>
        <?php foreach ($alumnos as $alumno): ?>
          <option value="<?= $alumno['idpersona']; ?>" 
            <?= $matricula['idalumno'] == $alumno['idpersona'] ? 'selected' : ''; ?>>
      ...
      <select name="idapoderado" id="idapoderado" class="form-select" required>
        <?php foreach ($apoderados as $apoderado): ?>
          <option value="<?= $apoderado['idpersona']; ?>" 
            <?= $matricula['idapoderado'] == $apoderado['idpersona'] ? 'selected' : ''; ?>>
    ...
    <div class="mb-3">
      ...
      <input type="text" class="form-control" id="parentesco" name="parentesco" 
             value="<?= $matricula['parentesco']; ?>" required>
    </div>
      ...
      <select name="idgrupo" id="idgrupo" class="form-select" required>
        <?php foreach ($grupos as $grupo): ?>
          <option value="<?= $grupo['idgrupo']; ?>" 
            <?= $matricula['idgrupo'] == $grupo['idgrupo'] ? 'selected' : ''; ?>>
    ...
    <div class="mb-3">
      ...
      <input type="date" class="form-control" id="fechamatricula" name="fechamatricula" 
             value="<?= $matricula['fechamatricula']; ?>" required>
    </div>

    <!-- Año escolar -->
    <div class="mb-3">
      <label for="anio_escolar" class="form-label">Año Escolar</label>
      <input type="number" class="form-control" id="anio_escolar" name="anio_escolar" 
             value="<?= $matricula['anio_escolar']; ?>" min="2000" max="2100" required>
    </div>
    ...
    <div class="mb-3">
      ...
      <select name="turno" id="turno" class="form-select" required>
        <option value="Mañana" <?= $matricula['turno'] == 'Mañana' ? 'selected' : ''; ?>>Mañana</option>
    ...
    <div class="mb-3">
      ...
      <select name="estado" id="estado" class="form-select" required>
        ...
        <option value="Inactivo" <?= $matricula['estado'] == 'Inactivo' ? 'selected' : ''; ?>>Inactivo</option>
      </select>
    </div>
